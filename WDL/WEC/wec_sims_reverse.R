...
OECD_sim = function () {
  ...
  highWlowE_1 = setDT(wec_dat_oecd_3)[,.SD[which.max(em_pc)],by = subsector]
  
  # second minimum emissions
  wec_dat_oecd_4 = wec_dat_oecd_3 %>% filter (!(ID %in% highWlowE_1$ID))
  highWlowE_2 = setDT(wec_dat_oecd_4)[,.SD[which.max(em_pc)],by = subsector]
  
  # third minimum emissions
  wec_dat_oecd_5 = wec_dat_oecd_4 %>% filter (!(ID %in% highWlowE_2$ID))
  highWlowE_3 = setDT(wec_dat_oecd_5)[,.SD[which.max(em_pc)],by = subsector]
  ...
  .GlobalEnv$highWlowE_rev = highWlowE
  ...
  # saving to environment
  .GlobalEnv$em_pc_totals_new_rev = em_pc_totals_new
...
Mid_income_sim = function () {
  ...
  midWlowE_1 = setDT(data3)[,.SD[which.max(em_pc)],by = subsector]
  
  # second minimum emissions
  data4 = data3 %>% filter (!(ID %in% midWlowE_1$ID))
  midWlowE_2 = setDT(data4)[,.SD[which.max(em_pc)],by = subsector]
  
  # third minimum emissions
  data5 = data4 %>% filter (!(ID %in% midWlowE_2$ID))
  midWlowE_3 = setDT(data5)[,.SD[which.max(em_pc)],by = subsector]
...
Low_income_sim = function () {
  ...
  # first minimum emissions
  data3 = data2 %>% filter (gdp > gdp_threshold)
  lowWlowE_1 = setDT(data3)[,.SD[which.max(em_pc)],by = subsector]
  
  # second minimum emissions
  data4 = data3 %>% filter (!(ID %in% lowWlowE_1$ID))
  lowWlowE_2 = setDT(data4)[,.SD[which.max(em_pc)],by = subsector]
  
  # third minimum emissions
  data5 = data4 %>% filter (!(ID %in% lowWlowE_2$ID))
  lowWlowE_3 = setDT(data5)[,.SD[which.max(em_pc)],by = subsector]
