...
DIOC_2000_clean = function () {
  ...
  DIOC_2000_1 = DIOC_2000_raw %>% 
    dplyr::select(-c("edu_cen", "age_cen", "reg_oecd")) %>%
    rename_with (
      ~ case_when (
        . == "country" ~ "dest",
        . == "coub" ~ "orig",
    ...
    mutate (age = ifelse (age == 1, "15_24",
                          ifelse (age == 2, "25_64",
                                  ifelse (age == 3, "65_", "unknown")))) %>%
    group_by (year, orig_dest, orig, region_birth, dest, edu_lvl, age) %>%
    summarise (migStock = sum (migStock))
...
DIOC_2005_clean = function () {
  ...
  DIOC_2005_1 = DIOC_2005_raw %>%
    dplyr::select(-c("edu_cen", "age_cen", "reg_oecd_2000", "reg_oecd_2010")) %>%
    rename_with (
      ~ case_when (
        . == "country" ~ "dest",
        . == "coub" ~ "orig",
  ...
  DIOC_2005_5$others = rowSums (DIOC_2005_5[,c(9,13:21)])
  
  DIOC_2005_6 = DIOC_2005_5 %>% dplyr::select (-c(9,13:21)) %>% relocate (others, .before = total_shares)
  
  # saving to Global Environment
  .GlobalEnv$DIOC_2005 = DIOC_2005_6
...
DIOC_2010_clean = function () {
  ...
  DIOC_2010_1 = DIOC_2010_raw %>%
    ...
    mutate (age = ifelse (age %in% c("15_24"), "15_24",
                          ifelse (age %in% c("25_34", "35_44", "45_54", "55_64"), "25_64", age))) %>%
    group_by (year, orig_dest, orig, region_birth, dest, edu_lvl, age) %>%
    summarise (migStock = sum (migStock))
  ...
  DIOC_2010_5$others = rowSums (DIOC_2010_5[,c(6:9,13,17:25)])
...
interpol_2015_20 = function () {
  ...
  DIOC_2000_20_1 [DIOC_2000_20_1$year %in% c(2015,2020), 6:12] = NA
  ...
  for (i in values) {
    ...
    data1 = data1 %>%
      ...
      mutate (pri_15_24_lagged = Lag (pri_15_24, -1)) %>%
      mutate (pri_15_24_growth = (pri_15_24_lagged - pri_15_24)/pri_15_24) %>%
      mutate (pri_15_24_growth = Lag (pri_15_24_growth, 1)) %>%
      
      mutate (sec_15_24_lagged = Lag (sec_15_24, -1)) %>%
      mutate (sec_15_24_growth = (sec_15_24_lagged - sec_15_24)/sec_15_24) %>%
      mutate (sec_15_24_growth = Lag (sec_15_24_growth, 1)) %>%
      
      mutate (ter_15_24_lagged = Lag (ter_15_24, -1)) %>%
      mutate (ter_15_24_growth = (ter_15_24_lagged - ter_15_24)/ter_15_24) %>%
      mutate (ter_15_24_growth = Lag (ter_15_24_growth, 1)) %>%
      
      mutate (pri_25_64_lagged = Lag (pri_25_64, -1)) %>%
      mutate (pri_25_64_growth = (pri_25_64_lagged - pri_25_64)/pri_25_64) %>%
      mutate (pri_25_64_growth = Lag (pri_25_64_growth, 1)) %>%
      
      mutate (sec_25_64_lagged = Lag (sec_25_64, -1)) %>%
      mutate (sec_25_64_growth = (sec_25_64_lagged - sec_25_64)/sec_25_64) %>%
      mutate (sec_25_64_growth = Lag (sec_25_64_growth, 1)) %>%
      
      mutate (ter_25_64_lagged = Lag (ter_25_64, -1)) %>%
      mutate (ter_25_64_growth = (ter_25_64_lagged - ter_25_64)/ter_25_64) %>%
      mutate (ter_25_64_growth = Lag (ter_25_64_growth, 1)) %>%
    ...
    pri_15_24_growth_avg = mean (data1$pri_15_24_growth, na.rm = T)
    sec_15_24_growth_avg = mean (data1$sec_15_24_growth, na.rm = T)
    ter_15_24_growth_avg = mean (data1$ter_15_24_growth, na.rm = T)
    pri_25_64_growth_avg = mean (data1$pri_25_64_growth, na.rm = T)
    sec_25_64_growth_avg = mean (data1$sec_25_64_growth, na.rm = T)
    ter_25_64_growth_avg = mean (data1$ter_25_64_growth, na.rm = T)
    others_growth_avg = mean (data1$others_growth, na.rm = T)
    
    data1$pri_15_24 [data1$year == 2015] <- data1$pri_15_24 [data1$year == 2010] * (1 + pri_15_24_growth_avg)
    data1$sec_15_24 [data1$year == 2015] <- data1$sec_15_24 [data1$year == 2010] * (1 + sec_15_24_growth_avg)
    data1$ter_15_24 [data1$year == 2015] <- data1$ter_15_24 [data1$year == 2010] * (1 + ter_15_24_growth_avg)
    data1$pri_25_64 [data1$year == 2015] <- data1$pri_25_64 [data1$year == 2010] * (1 + pri_25_64_growth_avg)
    data1$sec_25_64 [data1$year == 2015] <- data1$sec_25_64 [data1$year == 2010] * (1 + sec_25_64_growth_avg)
    data1$ter_25_64 [data1$year == 2015] <- data1$ter_25_64 [data1$year == 2010] * (1 + ter_25_64_growth_avg)
    data1$others [data1$year == 2015] <- data1$others [data1$year == 2010] * (1 + others_growth_avg)
    
    data1$pri_15_24 [data1$year == 2020] <- data1$pri_15_24 [data1$year == 2010] * (1 + pri_15_24_growth_avg)^2
    data1$sec_15_24 [data1$year == 2020] <- data1$sec_15_24 [data1$year == 2010] * (1 + sec_15_24_growth_avg)^2
    data1$ter_15_24 [data1$year == 2020] <- data1$ter_15_24 [data1$year == 2010] * (1 + ter_15_24_growth_avg)^2
    data1$pri_25_64 [data1$year == 2020] <- data1$pri_25_64 [data1$year == 2010] * (1 + pri_25_64_growth_avg)^2
    data1$sec_25_64 [data1$year == 2020] <- data1$sec_25_64 [data1$year == 2010] * (1 + sec_25_64_growth_avg)^2
    data1$ter_25_64 [data1$year == 2020] <- data1$ter_25_64 [data1$year == 2010] * (1 + ter_25_64_growth_avg)^2
...
# normalizing 2015 and 2020 rows

# checking total shares
DIOC_2000_20$total_shares <- rowSums (DIOC_2000_20 [,6:12])
DIOC_2000_20_1 = DIOC_2000_20
DIOC_2000_20_1 [,6:12] = DIOC_2000_20_1 [,6:12] / DIOC_2000_20_1 [,13]
DIOC_2000_20_1$total_shares_2 = rowSums (DIOC_2000_20_1[,6:12])
DIOC_2000_20_1 [is.na(DIOC_2000_20_1)] <- 0
DIOC_2000_20_1 = DIOC_2000_20_1 %>% 
  dplyr::select (-c("total_shares")) %>%
  rename (total_shares_2 = total_shares)

write.csv (DIOC_2000_20_1, file.path(output_path,"migStock_shares_edu_age.csv"), row.names = FALSE)
...
restructure = function () {
  ...
  distributions = DIOC_2000_20 %>% dplyr::select (-13)
  ...
  migStock_3 = migStocks_2 %>%
    ...
    group_by (year, dest) %>%
    summarise (mig_pri_15_24 = sum (mig_pri_15_24),
               mig_sec_15_24 = sum (mig_sec_15_24),
               mig_ter_15_24 = sum (mig_ter_15_24),
               mig_pri_25_64 = sum (mig_pri_25_64), 
               mig_sec_25_64 = sum (mig_sec_25_64), 
               mig_ter_25_64 = sum (mig_ter_25_64),
  ...
  .GlobalEnv$migStock = migStock_3
  
}

restructure()

write.csv (migStock, file.path(output_path,"v4_migStock_aft_multiplication.csv"), row.names = FALSE)
