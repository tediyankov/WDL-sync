...
DIOC_2000_clean = function () {
  ...
  DIOC_2000_3 = reshape2::dcast(DIOC_2000_2,
                                year + orig_dest + orig + region_birth + dest + age ~ edu_lvl,
                                value.var = "migStock")
  
  DIOC_2000_4 = reshape (DIOC_2000_3, 
  ...
  .GlobalEnv$DIOC_2000 = DIOC_2000_4
}
...
DIOC_2005_clean = function () {
  ...
  # converting long data into wide data 
  DIOC_2005_3 = reshape2::dcast(DIOC_2005_2,
                                year + orig_dest + orig + region_birth + dest + age ~ edu_lvl,
                                value.var = "migStock")
  
  DIOC_2005_4 = reshape (DIOC_2005_3, 
                         idvar = c("year", "orig_dest", "orig", "region_birth", "dest"),
                         timevar = "age",
                         direction = "wide") %>%
    rename_with (
  ...
  # saving to Global Environment
  .GlobalEnv$DIOC_2005 = DIOC_2005_4
...
DIOC_2010_clean = function () {
  ...
  # converting long data into wide data 
  DIOC_2010_3 = reshape2::dcast(DIOC_2010_2,
                                year + orig_dest + orig + region_birth + dest + age ~ edu_lvl,
                                value.var = "migStock")
  
  DIOC_2010_4 = reshape (DIOC_2010_3, 
                         idvar = c("year", "orig_dest", "orig", "region_birth", "dest"),
                         timevar = "age",
                         direction = "wide") %>%
    rename_with (
  ...
  # saving to Global Environment
  .GlobalEnv$DIOC_2010 = DIOC_2010_4
...
interpol_2015_20 = function () {
  ...
  # blanking 2020 rows 
  DIOC_2000_20_1 [DIOC_2000_20_1$year %in% c(2015,2020), 6:11] = NA
...
write.csv(DIOC_2000_20, file.path(output_path,"DIOC_2000_20_realStocks.csv"), row.names = FALSE)
...
restructure = function () {
  ...
  distributions = read.csv (file.path (output_path, "DIOC_2000_20_withflows_2.csv")) %>%
    select (1:11)
  ...
  migStock_3 = migStocks_2 %>%
    ...
    ungroup () %>%
    mutate (migStock_total = mig_pri_15_34 + mig_sec_15_34 + mig_ter_15_34 + mig_pri_35_64 + mig_sec_35_64 + mig_ter_35_64)
  
}

write.csv (migStock_3, file.path(output_path,"migStock_aft_multiplication.csv"), row.names = FALSE)

write.csv (migStocks_2, file.path(output_path,"migStock_bef_multiplication.csv"), row.names = FALSE)

migStocks_test = migStocks_2 %>%
  group_by (year, dest) %>%
  summarise (migStock = sum (migStock))

write.csv (migStock_3, file.path(output_path,"v2_migStock_aft_multiplication.csv"), row.names = FALSE)
